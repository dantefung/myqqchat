...
public class ChatUI extends JFrame implements ActionListener
{
	...
	public void InterFace()
	{
		...
		nameLabel.setFont(CommonData.getTitleFont());
		nameLabel.setBackground(qqColor);
		
		ImageIcon tImIcon = new ImageIcon("pic/chatUI/headImg.png");
		titlePctLabel = new JLabel(tImIcon);
		
		ImageIcon fontIcon = new ImageIcon("pic/chatUI/font.png");
		fontbtn = new JButton(fontIcon);
		
		ImageIcon fileIcon = new ImageIcon("pic/chatUI/file.png");
		...
		sendbtn.setFont(CommonData.getBtnFont());
		...
		cancelbtn.setFont(CommonData.getBtnFont());
		...
		itempan.add(filebtn);
		itempan.add(fontbtn);
		itempan.setSize(450, 30);
		itempan.setLocation(0, 300);
		itempan.setBackground(qqColor);
		
		inputTA.setBackground(qqColor);
		inputTA.setFont(new Font(CommonData.getHtmlFontName(), CommonData
		        .getHtmlFontStyle(), CommonData.getHtmlFontSize()));
...
//			    JScrollPane jScrollPane = new JScrollPane(ExPopMenu);
		...
		sendpan.setSize(450, 70);
		
		ImageIcon aokiIcon = new ImageIcon("pic/chatUI/aoki.png");
	...
	{
		// 窗体标题
		this.setTitle("I am " + CommonData.getName());
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		switch (actionStr)
		{
			case "发送":
			{
				...
				if (!inputText.equals(""))
				{
					String fontStyle = "font-family:"
					        + CommonData.getHtmlFontName()
					        + ";"
					        + "font-size:"
					        + CommonData.getHtmlFontSize()
					        + ";"
					        + "font-style:"
					        + fontStyleToString(CommonData.getHtmlFontStyle())
					        + ";"
					        + "color:"
					        + "#"
					        + towZeroHead(Integer.toHexString(CommonData
					...
					// 格式化聊天内容
					formatContent = "<font color = green>"
					        + CommonData.getName() + "  " + timeStr
					        + " :</font><br>" + chatContent;
					...
					if (isSingleChat)
					{
						...
						sendMsg = MessageTypeEnum.SingleChat.toString() + ":::"
						        + CommonData.getId() + ":::" + userId + ":::"
						        + formatContent;
						System.out
						        .println("send message one by one:" + sendMsg);
					}
					// 群聊
					else
					{
						...
						sendMsg = MessageTypeEnum.GroupChat.toString() + ":::"
						        + CommonData.getId() + ":::" + formatContent;
						System.out.println("gourp chat:" + sendMsg);
					}
					
					// 添加聊天内容到本地聊天窗口
					addChatContent("<font color = blue>" + CommonData.getName()
					        + " " + timeStr + " :</font><br>" + chatContent);
					
					// 发送消息至服务器
					ChatClientProxy.getInstance().sendRequest(
					        MessageTypeEnum.SingleChat, sendMsg);
					// 清空聊天窗口
					inputTA.setText("");
			...
			case "文件发送":
			{
				...
				{
					CommonData.setFileName(fc.getSelectedFile().getPath());
					String filename = fc.getSelectedFile().getName();
					ChatClientProxy.getInstance().sendRequest(
					        DataUtil.genMessage(MessageTypeEnum.FileSend,
					                userId, CommonData.getLocalHostIp(),
					                filename));
			...
			case "字体设置":
			{
				Font font = new Font(CommonData.getHtmlFontName(),
				        CommonData.getHtmlFontStyle(),
				        CommonData.getHtmlFontSize());
				FontUI fv = new FontUI(font, CommonData.getHtmlFontColor());
				fv.showDialog(this, getLocation().x + getWidth() / 2,
				        getLocation().y + getHeight() / 2);
				font = fv.getSelectedfont();
				CommonData.setHtmlFontName(font.getName());
				CommonData.setHtmlFontSize(font.getSize());
				CommonData.setHtmlFontStyle(font.getStyle());
				Color fvColor = fv.getSelectedcolor();
				CommonData.setHtmlFontColor(fvColor);
				
				inputTA.setFont(font);
				inputTA.setForeground(CommonData.getHtmlFontColor());
